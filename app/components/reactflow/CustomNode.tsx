  ...
  if (node_type?.toLowerCase() === 'trigger') {
    return (
      <div 
        ...
        style={{ backgroundColor: color || '#10b981', borderColor: color ? `${color}cc` : '#34d399' }}
      >
    ...
    return (
      ...
      >
        ...
        <div className="absolute right-0 top-1/4 translate-x-1/2 flex items-center gap-1">
          <Handle type="source" position={Position.Right} id="success" className="w-4 h-4 bg-green-500" />
        </div>
        {/* Failure / False Branch */}
        <div className="absolute right-0 bottom-1/4 translate-x-1/2 flex items-center gap-1">
          <Handle type="source" position={Position.Right} id="failure" className="w-4 h-4 bg-red-500" />
        </div>
      </div>
    );
  }

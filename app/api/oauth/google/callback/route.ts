/* ==============================================================================
   BLOCK COMMENT: HARDENED OAUTH CALLBACK ROUTE (XSS & CSRF MITIGATION)
   Validates HMAC-signed state, securely forwards token, and locks postMessage origin.
   ============================================================================== */
...
export async function GET(request: NextRequest) {
  ...
  if (!statePayload) {
    return NextResponse.json(
      { error: 'Invalid or expired OAuth state parameter (CSRF protection)' },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    // 2. Send tokens to backend passing session cookie for authorization
    ...
    // 3. Return sanitized HTML with strict postMessage target origin (preventing XSS & broad broadcast)

...
import { AlertCircle, Loader, Download, BarChart3 } from 'lucide-react';
...
export default function AdminAuditDashboard() {
  ...
  // Stats
  const stats = {
    totalSearches: logs.filter((l) => l.action === 'SEARCH').length,
    totalExports: logs.filter((l) => l.action === 'EXPORT').length,
    totalEdits: logs.filter((l) => l.action === 'EDIT').length,
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            { label: 'Exports/Prints', value: stats.totalExports, icon: '📥' },
            { label: 'Case Edits', value: stats.totalEdits, icon: '✏️' },
            { label: 'Active Users', value: stats.uniqueUsers, icon: '👥' },
          ].map((stat, i) => (
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Filter Audit Logs</h2>
          ...
          <div className="flex gap-3 mt-4">
            ...
            <button className="ml-auto flex items-center gap-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition font-medium text-sm">
              <Download size={16} />
              Export Report
            </button>
          </div>
